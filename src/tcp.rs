...
pub fn tcp_client(
    client_port: i32,
    server_port: i32,
    f: fn(&mut TcpStream, &mut TcpStream, id_connection: u16, streams_shared: Arc<Mutex<HashMap<u16, TcpStream>>>),
    ...
    for stream in listener.incoming() {
        match stream {
            Ok(stream) => {
                ...
                let mut stream_read = shared_map_ref.get(&id_cont).unwrap().try_clone().expect("Error clonning application stream");
                let mut server_write = to_server_stream.try_clone().expect("Error clonning");

                let shared_map_copy = shared_map.clone();
                thread::spawn(move || {
                    f(&mut stream_read, &mut server_write, id_cont, shared_map_copy);
                });
